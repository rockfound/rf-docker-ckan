...
def odata(context, data_dict):
    ...
    if t.request.GET.get('$sqlfilter'):
        ...
        logging.warning('DataStore Action=datastore_search_sql')
        ...
        logging.warning('DataStore Action=datastore_search_sql: ' + data_dict['sql'])
    else:
        ...
        logging.warning('DataStore Action=datastore_search: ' + data_dict['resource_id'])

    try:
        result = action({}, data_dict)
    except t.ObjectNotFound:
        t.abort(404, t._('OData DataStore resource not found:' + data_dict['resource_id']))
